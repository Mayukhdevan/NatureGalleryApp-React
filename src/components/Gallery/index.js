...
class Gallery extends Component {
  ...
  updateImageId = id => this.setState({activeId: id})

  render() {
    const {activeId} = this.state
    const activeImage = imagesList.filter(eachImg => eachImg.id === activeId)
        ...
        <div className="content-wrapper">
          <div className="img-container">
            <img
              ...
              alt={activeImage[0].imageAltText}
            />
          </div>
          ...
          <ul className="thumbnail-container">
            {imagesList.map(eachThumbnail => (
              <ThumbnailItem
                ...
                thumbnailItem={eachThumbnail}
                updateImageId={this.updateImageId}
                isActive={eachThumbnail.id === activeId}
              />
            ))}
          </ul>
        </div>
